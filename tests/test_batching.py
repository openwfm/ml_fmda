# Module to test batching of data for RNNs
...
n_samples = 5
data = np.arange(n_samples + 10) 
X_batch_1 = np.lib.stride_tricks.sliding_window_view(data, window_shape=12)[:n_samples, :, None]
n_samples = 3
data = np.arange(n_samples + 10) 
X_batch_2 = np.lib.stride_tricks.sliding_window_view(data, window_shape=12)[:n_samples, :, None]
...
if __name__ == '__main__':
    ...
    # X1 = data_funcs.staircase(test_1, sequence_length = seq_length)
    ...
    # X2 = data_funcs.staircase(test_2, sequence_length = seq_length)
    ...
    # X3 = data_funcs.staircase(test_3, sequence_length=seq_length)
    # assert X3.shape[0] == 1, f"TEST FAILED, expected 1 sample but got {X3.shape[0]}"

    # print("Testing Batch Construction with Staircase")
