...
\Title{A Recurrent Neural Network for Forecasting Dead Fuel Moisture Content with Inputs from Numerical Weather Models}
...
\end{adjustwidth}
\caption{
Rocky Mountain GACC – 161 RAWS with valid data from 2023 through 2024. (left) The National view is rendered in Web Mercator (EPSG:3857). (right) The regional view is plotted using a Lambert Conformal Conic projection 
...
\midrule
Solar Radiation & $\text{W\,m}^{-2}$ & Downward shortwave radiative flux. & 220.99 & 0.00 & 1130.00 \\
\midrule
Wind Speed & $\text{m\,s}^{-1}$ & Wind speed at 10m. & 4.70 & 0.13 & 32.50 \\
\midrule
...
\midrule
Rain & $\text{mm\,h}^{-1}$ & Calculated from rain accumulated over the hour. & 0.04 & 0.00 & 68.29 \\
\midrule
...
We collected all available FMC data for 2023 through 2024 within the Rocky Mountain Geographic Area Coordination Center (GACC), a geographically diverse region in the central United States. The spatial bounding box for this region is from Latitude 37 N to Latitude 46 N, and from Longitude -111 W to Longitude -95 W. This region includes parts of Colorado, Wyoming, Nebraska, Kansas, Montana, and South Dakota. There were 161 total RAWS with valid FMC data in this period and spatial domain, and 151 total RAWS with valid data in the forecasting period of 2024. Figure~\ref{fig:st_maps} shows the location of the RAWS within the Rocky Mountain GACC with valid FMC data from 2023-2024. There is a greater density of RAWS around the heavily populated area of Denver, Colorado, as well as in the Black Hills National Forest near Rapid City, South Dakota. The Rocky Mountain region is particularly prone to wildfire danger and numerous federal, state, and local agencies are involved with wildfire management. The atmospheric weather predictors were chosen based on physical considerations and based on inputs to other models of FMC in the scientific literature. A complete list of all data variables used in this study is listed in Table~\ref{tab:all_vars}.
...
\caption{A single recurrent unit takes as input weather data and a recurrent state (left). An LSTM layer and subsequent dense layers forms the recurrent unit (right). For each LSTM cell, the recurrent state has two elements: the short term hidden state and the long term cell state.}
\label{fig:rnn}
\end{figure}   
...
The accuracy of the RNN model and three baseline models was evaluated when predicting values at unobserved locations across all of 2024 in 48-hour forecast windows. The models were evaluated on the same randomly selected test locations across 500 random samples of locations. Replications had different combinations of RAWS in the test set, which each have different amounts of data availability across the testing period of 2024. A single replication had 16 test RAWS that were not included in training, and on average roughly 92,300 individual FMC predictions. In total, 46,165,248 individual FMC predictions were generated across all hours of 2024. Table~\ref{tab:overall} shows the RMSE when averaged over all times, locations, and replications. The bounds on the estimated metrics are plus or minus one standard deviation of the given metric across all replications.The uncertainty presented in Table~\ref{tab:overall} represents the uncertainty in the overall RMSE estimate when we vary things under our direct control, namely the RAWS used for training and testing and the random initial weights of the RNN. The uncertainty bounds represent the uncertainty of the overall RMSE estimate. The RNN achieved the lowest forecast error compared to the baseline methods as indicated by the RMSE.  Further, the RMSE of the RNN is well below the uncertainty bounds of the other methods. Thus, there is strong statistical evidence that the RMSE of the RNN is lower than the other methods for 2024 in the Rocky Mountain region. 
...
\caption{Observed (left) and predicted (right) FMC distributions. The x-axis indicates FMC (\%) and the y-axis indicates the frequency of occurrences. Across 500 replications, there were a total of 46,165,248 individual FMC predictions generated, with an average of 92,300 predictions per replication.}
\label{fig:hists}
\end{figure}   
...
\begin{figure}[tbh]
\includegraphics[width=10 cm]{Definitions/err24}
...
\label{fig:err24}
\end{figure}   
\subsection{Predictor Sensitivity Analysis}
\label{sec:sens}
To evaluate the contributions of the different predictors in the RNN model, we conduct a sensitivity analysis that removes different predictors and compares the effect on the forecast error. The full model used predictors from the HRRR weather model, including wetting equilibrium, drying equilibrium, solar radiation, wind speed, and rain. Geographic predictors from the physical location of the RAWS included elevation, longitude, and latitude. Finally, predictors from the time of observation were hour of the day and day of the year. See Table~\ref{tab:all_vars} for more information on the individual predictors. We rerun the forecast analysis described in Section~\ref{sec:cv} using different combinations of these predictors. All other model configurations and hyperparameters are unchanged. It should be noted that these predictors are all related to each other. Weather conditions vary in space and time, and the weather variables themselves are related to each other. The purpose of the sensitivity analysis in this section is to give a general indication of which predictors are driving the RNN model performance, and also to evaluate whether there is any sign of overfitting where the model accuracy improves if any predictors are removed.
...
\begin{table}[tbh]
\caption{Sensitivity analysis for predictor groups. The overall estimated RMSE for each model configuration is averaged over all replications, all locations, and all times. The relative difference divides the difference by the baseline estimate from the full model\label{tab:sens1}}
...
\begin{table}[tbh]
\caption{Sensitivity analysis for specific predictors. The overall estimated RMSE for each model configuration is averaged over all replications, all locations, and all times.\label{tab:sens2}}
\begin{tabularx}{\textwidth}{CCCC}
\toprule
...
\subsection{Case-Study - The Alexander Mountain Fire, Colorado, Summer 2024}
\label{sec:case}
...
\begin{figure}[tbh]
\includegraphics[width=10 cm]{Definitions/alexander_map}
\caption{The 2024 Alexander Mountain Fire perimeter from NIFC and 4 nearest RAWS.\label{fig:alexmap}}
\end{figure}  
\begin{table}[tbh]
\caption{RAWS within Alexander Mountain Fire area with valid FMC data during pre-fire period.}
\label{tab:case_raws}
\begin{tabularx}{\textwidth}{CCCC}
\toprule
...
\begin{table}[tbh]
\caption{RNN forecast accuracy during pre-fire period, July 25, midnight to July 27 23:00, Mountain time.}
...
\begin{adjustwidth}{-\extralength}{0cm}
\centering
\includegraphics[width=\linewidth]{Definitions/case_study_rmse_hist}
\end{adjustwidth}
% \includegraphics[width=12 cm]{Definitions/case_study_rmse_hist}
\caption{Distribution of RMSE for four stations near the Alexander Mountain Fire across replications.  \label{fig:casehists}}
\end{figure}  
...
\begin{adjustwidth}{-\extralength}{0cm}
\centering
\includegraphics[width=\linewidth]{Definitions/case_ts}
\end{adjustwidth}
\caption{Forecasted and observed FMC (top) and some weather inputs (bottom) for RAWS BEYC2 during pre-fire period for Alexander Mountain Fire.\label{fig:casets}}
\end{figure}  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Discussion}
% what was done here
In this study, we compared an RNN model of dead 10-h fuel moisture content with weather inputs from HRRR to several baseline methods. The accuracy of the models was compared when forecasting 48 hours into the future and at unobserved locations for all of 2024 in the Rocky Mountain region. The model is the first continent-wide ML model of dead FMC that is compatible with real-time forecasting. The rigorous spatiotemporal cross-validation approach to estimate forecast accuracy is the first large-scale study of its kind for FMC modeling, either alive or dead. The case-study of Alexander Mountain in Colorado in 2024 shows that the RNN forecasts can provide reliable indications of pre-fire danger. The RNN was the most accurate model of FMC compared to the baseline methods, namely a physics-based model, a non-recursive ML model, and a climatology method, as shown in Section \ref{sec:results}. This model should be interpreted as forecasting what an FMC sensor will say in the future. We do not attempt to correct for systematic errors between the FMC sensors and gravimetric FMC measurements. FMC sensors are already in operational use around the world, and the forecasts from the RNN are intended to be used in place of those measurements at future times.

% elaborate on accuracy
The overall estimated RMSE for the model was $3.02\%$. The case-study presented in Section~\ref{sec:case} showed that this overall forecast accuracy is representative of the granular forecast error at locations and times that represented extreme fire danger.
...
\funding{The research presented in this paper was partially funded by NASA grants 80NSSC22K1717, 80NSSC23K1118, 80NSSC22K1405, 80NSSC23K1344, and 80NSSC25K7276.
...
\acknowledgments{A portion of this work used code generously provided by Brian Blaylock's Herbie Python packages Herbie (Version 2024.8.0) (\url{https://github.com/blaylockbk/SynopticPy}) and SynopticPy  (\url{https://github.com/blaylockbk/SynopticPy}). Large Language Models, specifically ChatGPT-5, were utilized for support with coding and literature search.}
